...
- **视觉避障**：头部高度 5 条 raycast（±54°/±29°/0°，视距 1.3 m）测遮挡距离，编码进视叶
  VISL/VISR 群（200 神经元中的 8 个，192-199）。单侧障碍 → 对侧转向群兴奋、
  同侧抑制（左前有石头 → 右转绕开）；正面对称接近 → 招募 S 群停下，站立稳态压
  习惯化后重新出发绕行。物理端同样的几何作为碰撞体注入 MJCF
  （`physics.js` 的 `MAZE_WALLS` / `OBSTACLES`），raycast 看见的 = 身体撞得上的
- **触觉诱导定向（迷宫逃生关键回路）**：VISL/VISR → A（联合群）在持续视觉接触时
  解锁原地转向门控（`wz` 门 = max(行走驱动, A 群活性)），外加一点定向爬行速度——
  死端不再是"停-走-蹭墙"循环，而是原位弧线掉头；开阔的一侧没有视觉制动，
  掉头自然 settle 朝向缺口
- **迷宫逃生**：`MAZE_WALLS` 定义一个 L 形迷宫（长臂 2.4 m 宽 + 南侧死胡同口袋，
  唯一出口在短臂北端）。走廊宽度恰好让居中的侧墙落在 1.3 m 视距之外——
  居中时"失明"，偏向一侧时该侧墙进入视野、回避转向自动扶正，涌现出蛇形居中。
  出生点在长臂深处，无地图、无记忆、无规划，纯靠上述回路的探索-回避-定向。
  2-D 闭环统计（`tools/maze_sim.mjs`，20 随机种子）：**~70% 在 3 分钟内逃出，
  中位 ~51 s**；浏览器真实物理（MuJoCo + RL 策略）实测多次逃出
...
└── tools/
    ...
    └── maze_sim.mjs                # 2-D 闭环迷宫模拟器 (node tools/maze_sim.mjs, 调参用)
```
